...
class TodoModel {
    ...
    public function getAllTodos() {
        $query = "SELECT * FROM " . $this->table_name . " 
                  ORDER BY 
                  CASE 
                      WHEN is_completed = 0 AND deadline IS NOT NULL THEN 0
                      WHEN is_completed = 0 AND deadline IS NULL THEN 1
                      ELSE 2
                  END,
                  deadline ASC,
                  created_at DESC";
     ...
     * @return boolean
     */
public function createTodo($task, $deadline = null) {
    $query = "INSERT INTO " . $this->table_name . " (task, deadline) 
